...
## FAQ
* 仓库在哪里？
  * 答：[在这里](https://github.com/arceos-usb/arceos_experiment/tree/usb-camera-base)，注意分支是usb-camera-base

## 前言-系统架构

![structure](resource/img/usb_main_architecture.png)
...
* 驱动无关的设备抽象：usb协议的上层建筑，各种驱动千变万化，但是它们都有着共同的操作对象，因此我们在这里将所有驱动都会用到的设备数据抽象出来，进一步的说，其内包含：
  ...
  * 设备当前所使用的interface与config值与端点的概念
    ...
    * endpoint-设备端点-对应着图中的端点描述符，“端点”在usb系统中，是主机与设备沟通的最小通道，端点本身也是一种状态机
      ...
      ![endpoint state machine](resource/img/endpoint-state-machine.png)
...
* 驱动api：可以在图中看到，我们的驱动部分是可扩展的，甚至可以动态的加载驱动模块，有着非常巧妙的设计：
  ...
  * 首先，我们的驱动采用部分工厂模式+状态机的思想，其api定义如下：
    ...
    * 也就是说，在正常运行的前提下，我们整个usb系统都是在不断的进行"tick-tock"，就像是时间轮一样：
      ...
      ![tick-tock-machine](resource/img/tick-tock-machine.png)
...
* Q: 大概明白了，但是具体来说，怎么从拿到手的一堆Hex数据中读取出这些东西呢？
  * A: 照着规范来，各个字段的标志位都是有约定的，具体来说，参考[官方文档](./resource/img/hut1_2.pdf)与[社区总结](https://www.usbzh.com/article/forum-80.html)
